 ...
 *     samym `entitlement_dedupe_key` (`${ENTITLEMENT_DEDUPE_ON_CONFLICT_CLAUSE}`)
 *     jest NO-OP-em (0 affected) — partial UNIQUE index nie podwaja entitlementu;
 *     N recipientów na RÓŻNYCH liniach ⇒ N osobnych wierszy (FR10 happy).
 *   (C1 deferowany / korekta V1) multi-recipient na TEJ SAMEJ linii: drugi recipient
 *     ma różny `dedupe_key`, więc `ON CONFLICT(dedupe_key)` GO NIE ŁAPIE — narusza
 *     partial UNIQUE `(order_id, line_item_id)` (v1.9.0 H-6) ⇒ `unique_violation`
 *     (DB fail-loud). De-mask: test aplikuje PEŁNY łańcuch migracji (łącznie z
 *     `1778925500000`), więc kolizja jest UJAWNIONA, nie maskowana okrojonym schematem.
 *
...
import { Migration1778925500000 } from "../../modules/voucher/migrations/1778925500000_add_line_item_id_to_entitlement_instance"
...
maybe("Story 3.3 — live-issue na realnym PG (AC3 vat-stability + AC4 dedupe)", () => {
  ...
  async function withMigratedTx(fn: (c: PoolClient) => Promise<void>): Promise<void> {
    ...
    try {
      ...
      // PEŁNY łańcuch migracji entitlement_instance (de-mask V1): łącznie z
      // `1778925500000`, które tworzy partial UNIQUE `(order_id, line_item_id)`
      // (v1.9.0 H-6). Pominięcie go maskowało kolizję multi-recipient-per-line.
      for (const sql of [
        ...collectUpSql(Migration1778925500000),
  ...
  it("(AC4/FR10 happy) N recipientów na RÓŻNYCH liniach ⇒ N osobnych wierszy", async () => {
    // FR10 happy-path: każdy recipient na WŁASNEJ linii (1 recipient/line_item).
    // Różny `line_item_id` ⇒ różny (order_id, line_item_id) ⇒ partial UNIQUE
    // `(order_id, line_item_id)` (v1.9.0 H-6) NIE jest naruszany ⇒ N wierszy.
    await withMigratedTx(async (c) => {
        ...
        c.query(
          `INSERT INTO entitlement_instance
             (id, entitlement_profile_id, entitlement_type, order_id, line_item_id,
              market_id, sales_channel_id, vat_classification, entitlement_dedupe_key,
              recipient_index, state, policy_snapshot)
           VALUES ($1,'p','VOUCHER_SERVICE','order_m',$2,'bonbeauty','sc_1','MPV',$3,0,
                   'ISSUED','{}'::jsonb)
           ${ENTITLEMENT_DEDUPE_ON_CONFLICT_CLAUSE}`,
          [`ent_${lineId}`, lineId, buildEntitlementDedupeKey("pi_multi", lineId, 0)]
        )
      expect((await insert("li_a")).rowCount).toBe(1)
      expect((await insert("li_b")).rowCount).toBe(1)
      expect((await insert("li_c")).rowCount).toBe(1)
  ...
  it("(C1 deferowany / V1 de-mask) multi-recipient na TEJ SAMEJ linii ⇒ unique_violation order_line (DB fail-loud, NIE ciche podwojenie)", async () => {
    // Dowód, że odgrodzenie C1 NIE jest maskowane: na PEŁNYM schemacie (z
    // `1778925500000`) drugi recipient na (order_c1, li_same) — różny dedupe_key,
    // więc `ON CONFLICT (entitlement_dedupe_key)` GO NIE ŁAPIE — narusza partial
    // UNIQUE `entitlement_instance_order_line_uniq_idx (order_id, line_item_id)` ⇒
    // Postgres rzuca `unique_violation` (fail-loud), zamiast cicho podwoić/skolidować.
    // To DB-warstwowy backstop dla writer-owego guarda `recipients.length > 1`.
    await withMigratedTx(async (c) => {
        ...
        c.query(
          `INSERT INTO entitlement_instance
             (id, entitlement_profile_id, entitlement_type, order_id, line_item_id,
              market_id, sales_channel_id, vat_classification, entitlement_dedupe_key,
              recipient_index, state, policy_snapshot)
           VALUES ($1,'p','VOUCHER_SERVICE','order_c1','li_same','bonbeauty','sc_1','MPV',$2,$3,
                   'ISSUED','{}'::jsonb)
           ${ENTITLEMENT_DEDUPE_ON_CONFLICT_CLAUSE}`,
          [`ent_c1_${idx}`, buildEntitlementDedupeKey("pi_c1", "li_same", idx), idx]
        )
      // recipient_index=0: pierwszy wiersz na (order_c1, li_same) — PRZECHODZI.
      expect((await insert(0)).rowCount).toBe(1)
      // recipient_index=1: różny dedupe_key (ON CONFLICT na innej kolumnie NIE łapie),
      // ale (order_c1, li_same) już zajęte ⇒ narusza order_line UNIQUE ⇒ rzut.
