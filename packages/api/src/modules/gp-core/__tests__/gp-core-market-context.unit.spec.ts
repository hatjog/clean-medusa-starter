...
import GpCoreService, { MarketContextRequiredError, resolveGpCoreRlsEnforced } from "../service";
...
describe("GpCoreService — gp_core market transaction context", () => {
  const originalRlsEnforced = process.env.GP_CORE_RLS_ENFORCED

  afterEach(() => {
    if (originalRlsEnforced === undefined) {
      delete process.env.GP_CORE_RLS_ENFORCED
    } else {
      process.env.GP_CORE_RLS_ENFORCED = originalRlsEnforced
    }
  })
  ...
  it.each([
    ...
    ["true", true],
    ["1", true],
  ])("parses GP_CORE_RLS_ENFORCED=%p as %p", (raw, expected) => {
    expect(resolveGpCoreRlsEnforced(raw)).toBe(expected)
  })
  ...
  it("keeps default flag OFF: withTransaction does not set runtime role", async () => {
    delete process.env.GP_CORE_RLS_ENFORCED
    const service = new GpCoreService({}, {});
    const queries: Array<{ sql: string; params?: unknown[] }> = [];
    const client = {
  ...
  it("sets gp_core runtime role transaction-locally before market context when flag is ON", async () => {
